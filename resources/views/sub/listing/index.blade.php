...
<div id="wrapper">
    <div class="content">
            ...
            <div class="categoties-column_container cat-list">
                <ul>
                    ...
                    @foreach ($categories as $category)
                        <li><a href="{{$category->url(get_subdomain())}}"><i class="" style="font-size: 12px;">{{$category->title}}</i><span>{{$category->title}}</span></a></li>
                    @endforeach
                    --}}
                </ul>
            </div>
        ...
        <div class="top-search-content">
            <form action="{{ route('sub.listing.index',get_subdomain()) }}" class="top-search-content">
            ...
            <div class="more-search-opt-wrap more-hidden_wrap">
                ...
                <div class="msotw_footer">
                    <a href="#" class="btn small-btn float-btn color-bg">Ara</a>
                    <div class="reset-form reset-btn"> <i class="far fa-sync-alt"></i> Filtreleri Sıfırla</div>
                </div>
            </div>
            </form>
        </div>
        ...
        <div class="col-list-wrap gray-bg ">
            ...
            <div class="listing-item-container fl-wrap">
                @foreach($listings as $key=>$listing)
                    @include('sub.partials.listing-vertical',['_listing'=>$listing,'listing_key'=>$key+1])
                @endforeach   
                @if(!$listings->count())
        ...
        function mainMap() {
            ...
            var locations = [
                @foreach($listings as $_listing)
                    @include('sub.partials.location-data',['_listing'=>$_listing])
                @endforeach
            ];
            //   Map Infoboxes end ------------------
